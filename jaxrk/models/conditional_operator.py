...
from ..rkhs.base import LinOp, RkhsObject, Vec, InpVecT, OutVecT, RhInpVectT, CombT
from ..rkhs.cov import *
from ..rkhs.operator import FiniteOp


def Cmo(inp_feat: InpVecT, outp_feat: OutVecT,
        regul: float = None) -> FiniteOp[InpVecT, OutVecT]:
...
def RidgeCmo(inp_feat: InpVecT, outp_feat: OutVecT,
             regul: float = None) -> FiniteOp[InpVecT, OutVecT]:
    if regul is None:
...
def Cdo(inp_feat: InpVecT, outp_feat: OutVecT, ref_feat: OutVecT,
        regul=None) -> FiniteOp[InpVecT, OutVecT]:
    if regul is not None:
        regul = np.array(regul, dtype=np.float32)
        assert regul.squeeze().size == 1 or regul.squeeze(
        ).shape[0] == len(inp_feat)
