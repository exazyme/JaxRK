...
def gp_loglhood_mean0_univ(
        y: Array,
        cov_chol: Array,
        prec_y: Array = None) -> float:
...
def gp_val_loss(train_sel: Array,
    ...
    chol, y, prec_y, ymean, ystd, noise = gp_init(
        x_train, y_train, noise, True)
...
vmap_gp_val_rank = jax.jit(
    jax.vmap(
        partial(
            gp_val_loss, loss=UcbRankLoss(
                condis_loss, 0.)), (0, 0, None, None, None)))
...
def gp_cv_val_lhood(train_val_idcs: Array,
                    x_inner_x: Array,
                    y: Array,
                    regul: float = None,
                    vmapped_loss: Callable[[Array,
                                            Array,
                                            Array,
                                            Array,
                                            float],
                                           Array] = vmap_gp_val_lhood):
    if y.ndim == 1:
        y = y[:, np.newaxis]
...
class GP(object):
    ...
    def predict(self, xtest: FiniteVec, diag=True):
        pred_m, pred_cov = gp_predictive(self.x.inner(
            xtest), xtest.inner(), self.chol, self.prec_y, self.ymean, self.ystd)
        if diag:
