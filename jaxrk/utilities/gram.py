...
def rkhs_gram_cdist(
    ...
    if G_a is not None:
        assert len(
            G_a.shape) == 2 and G_a.shape[0] == G_a.shape[1] and G_ab.shape[0] == G_a.shape[0]
    ...
    if G_a is None or G_b is None:
        assert G_a is None and G_b is None, 'Either none or both of G_repr, G_orig should be None'
...
def rkhs_gram_cdist_unchecked(
        ...
        G_b: np.array,
        power: float = 2.):
    sqdist = np.diagonal(G_a)[:, np.newaxis] + \
        np.diagonal(G_b)[np.newaxis, :] - 2 * G_ab
    if power == 2.:
        return sqdist
    else:
        return np.power(sqdist, power / 2.)
...
def gram_projection(
        G_orig_repr: np.array,
        G_orig: np.array = None,
        G_repr: np.array = None,
