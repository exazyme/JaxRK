...
def test_gp_init(D_X, D_Y, kernel, N):
    ...
    chol, y_rescaled, prec_y, ymean, ystd, noise = gp.gp_init(v_X.inner()+np.eye(N)*2, y, None, True)
    y_p_m, y_p_cov = gp.gp_predictive(v_X.inner(v_X_p), v_X_p.inner(), chol, prec_y, ymean, ystd)
    y_p_m, y_p_cov, lhood = gp.gp_predictive(v_X.inner(v_X_p), v_X_p.inner(), chol, prec_y, ymean, ystd, np.zeros((N, D_Y)))
    #print(lhood)
