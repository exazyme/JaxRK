...
def test_apply_matmul():
    ...
    x_fv = FiniteVec(gk_x, np.vstack([x, y]), reduce=[
                     LinearReduce(diag_stacked_factors), BalancedRed(x.size)])
    ...
    res_e2 = (oper @ x_e2)
    res_v = (oper @ x_fv)
    assert np.allclose(res_e1.reduce[0].linear_map, (oper.matr @ oper.inp_feat.inner(
        x_e1)).flatten()), "Application of operator to RKHS element failed."
    assert np.allclose(
        res_v.insp_pts, res_e1.insp_pts), "Application of operator to all vectors in RKHS vector failed at inspace points."
    assert np.allclose(res_v.reduce[0].linear_map, np.vstack(
        [res_e1.reduce[0].linear_map, res_e2.reduce[0].linear_map])), "Application of operator to all vectors in RKHS vector failed."
    assert np.allclose((oper @ oper).matr, oper.inp_feat.inner(oper.outp_feat)
                       ), "Application of operator to operator failed."
...
def test_FiniteOp():
    ...
    assert (
        np.allclose(
            (C2 @ ref_elem).reduce[0].linear_map,
            np.sum(
                C1.matr,
                0)))
...
def test_CovOp(plot=False):
    ...
    targ = mixt(D, [multivariate_normal(3 *
                                        np.ones(D), np.eye(D) *
                                        0.7**2), multivariate_normal(7 *
                                                                     np.ones(D), np.eye(D) *
                                                                     1.5**2)], [0.5, 0.5])
    ...
    assert (np.abs(targp.squeeze() - estp).mean() <
            0.8), "Estimated density strongly deviates from true density"
    if plot:
        pl.plot(ref_fvec.insp_pts.squeeze(), estp / np.max(estp)
                * np.max(targp), "b--", label="scaled estimate")
        pl.plot(ref_fvec.insp_pts.squeeze(), estp2 / np.max(estp2) *
                np.max(targp), "g-.", label="scaled estimate (uns)")
...
def generate_donut(nmeans=10, nsamps_per_mean=50):
    ...
    rvs = comp_distribution.rvs(
        nmeans * nsamps_per_mean) + np.repeat(means, nsamps_per_mean, 0)

    def true_dens(samps): return exp(
        location_mixture_logpdf(
            samps,
            means,
            np.ones(nmeans) /
            nmeans,
...
def test_Cdmo(plot=False):
    ...
    for center in cent_vals:
        ...
        print(
            np.abs(
                cm.const_cent_term -
                maps[center]["dens"].const_cent_term).max())
    ...
    t = np.array([true_dens(np.hstack(
        [np.repeat(x, len(refervec.insp_pts), 0), refervec.insp_pts])) for x in x_vals])
    if plot:
            ...
            for map_type in ["emb", "dens"]:
                for cent in cent_vals:
                    ...
                    if cent:
                        ...
                        style = "--"
                    ax[i].plot(refervec.insp_pts,
                               ests[map_type][cent][i],
                               style,
                               color=color,
                               label=map_type + " " + ("cent" if cent else "unc"),
                               alpha=0.5)
