...
kernel_setups = [GenGaussKernel.make_gauss(1.0)]
...
def test_apply_matmul():
    ...
    x_fv = FiniteVec(
        gk_x,
        np.vstack([x, y]),
        reduce=[LinearReduce(diag_stacked_factors), BalancedRed(x.size)],
    )
    ...
    res_v = oper @ x_fv
    assert np.allclose(
        res_e1.reduce[0].linear_map, (oper.matr @ oper.inp_feat.inner(x_e1)).flatten()
    ), "Application of operator to RKHS element failed."
    assert np.allclose(
        res_v.insp_pts, res_e1.insp_pts
    ), "Application of operator to all vectors in RKHS vector failed at inspace points."
    assert np.allclose(
        res_v.reduce[0].linear_map,
        np.vstack([res_e1.reduce[0].linear_map, res_e2.reduce[0].linear_map]),
    ), "Application of operator to all vectors in RKHS vector failed."
    assert np.allclose(
        (oper @ oper).matr, oper.inp_feat.inner(oper.outp_feat)
...
def test_CovOp(plot=False):
    ...
    targ = mixt(
        D,
        [
            multivariate_normal(3 * np.ones(D), np.eye(D) * 0.7**2),
            multivariate_normal(7 * np.ones(D), np.eye(D) * 1.5**2),
        ],
        [0.5, 0.5],
    )
    ...
    ), "Estimated density strongly deviates from true density"
    if plot:
        pl.plot(
            ref_fvec.insp_pts.squeeze(),
    ...
    assert (
        np.std(supp) < 0.15
    ), "Estimated support has high variance, in data points, while it should be almost constant."
    ...
    def true_dens(samps):
        return exp(
            location_mixture_logpdf(
                samps, means, np.ones(nmeans) / nmeans, comp_distribution
            )
        )
...
def test_Cdmo(plot=False):
    ...
    ests = {
        map_type: {
            cent: np.array(
                [
                    maps[cent][map_type](x).dens_proj()(refervec.insp_pts).squeeze()
                    for x in x_vals
                ]
            )
    ...
    t = np.array(
        [
            true_dens(
                np.hstack([np.repeat(x, len(refervec.insp_pts), 0), refervec.insp_pts])
            )
            for x in x_vals
        ]
    )
