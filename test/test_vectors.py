...
def test_FiniteVec(D, kernel, N):
    ...
    rv2 = FiniteVec.construct_RKHS_Elem(
        kernel,
        rng.randn(
            N +
            1,
    ...
    assert np.allclose(
        inner(
            rv, rv), rv.k(
            rv.insp_pts, rv.insp_pts) * np.outer(
                rv.reduce[0].prefactors, rv.reduce[0].prefactors)), "Simple vector computation not accurate"
    assert np.allclose(
        inner(
            rv, rv2), (rv.k(
                rv.insp_pts, rv2.insp_pts) * np.outer(
                rv.reduce[0].prefactors, rv2.reduce[0].linear_map)).sum(
...
def test_Mean_var(kernel):
    ...
    el = FiniteVec.construct_RKHS_Elem(kernel, np.array(
        [(0.,), (1.,)]), prefactors=np.ones(2) / 2)
    ...
    el = FiniteVec.construct_RKHS_Elem(kernel, np.array(
        [(0.,), (1.,)]), prefactors=np.array([1. / 3, 2. / 3]))
    for pref in [el.reduce[0].linear_map, 2 * el.reduce[0].linear_map]:
        el.reduce[0].linear_map = pref
        m, v = el.normalized().get_mean_var()
        # print(m,v)
    ...
    el = FiniteVec.construct_RKHS_Elem(kernel, np.array(
        [(0.,), (1.,), (2., )]), prefactors=np.array([0.2, 0.5, 0.3]))
    for pref in [el.reduce[0].linear_map, 2 * el.reduce[0].linear_map]:
        el.reduce[0].linear_map = pref
        m, v = el.normalized().get_mean_var()
        # print(m,v)
...
def test_point_representant(kernel):
    ...
    assert np.allclose(vec.point_representant(
        "inspace_point").squeeze(), np.array([1., 1.]))
    assert np.allclose(vec.point_representant(
        "mean").squeeze(), np.array([.6, 2. / 3]))
...
def test_pos_proj(kernel):
    ...
    for v in vecs:
        assert np.allclose(
            v(x), v.pos_proj()(x), atol=5e-5), "Error tolerance not met for positive projection of RKHS vector, maximum absolute error was " + str(
            np.abs(
                v(x) - v.pos_proj()(x)).max())
