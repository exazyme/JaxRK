...
def test_dist():
    ...
    print(
        "Testing distance computation between vectors of RKHS elements",
        file=sys.stderr)
    k = LinearKernel()
    ...
    for pow in [2, 1]:
        for insp_args, dist_args in [[(a, b), (va, vb)], [(a, a), (va, va)]]:
            lin_rkhsdist_sq = (
                np.diag(
                    insp_args[0] @ insp_args[0].T)[
                    :,
                    np.newaxis] -
                2 *
                insp_args[0] @ insp_args[1].T +
                np.diag(
                    insp_args[1] @ insp_args[1].T)[
                    np.newaxis,
                    :])
...
def test_inv_blockmatr():
    G = GenGaussKernel.make_gauss(1.)(
        np.array([[1, 2, 3, 4]]).T + np.eye(4) * 3)
