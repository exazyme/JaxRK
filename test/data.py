...
class DataPrep(object):
    ...
    def get_data(
        ...
        if not exists(pickle_name):
            rval = preparation_func(
                DataPrep.download_check_unpack(
                    url, md5, *contained_file_paths))
...
class Power(object):
    ...
    def __init__(self, root='~/Documents/datasets/'):
        ...
        self.data = DataPrep.get_data(
            ...
            lambda file_objs: Power.prepare(
                Power.load_raw(
                    file_objs["household_power_consumption.txt"])),
            pickle_name,
    ...
    def get_ihgp_window(self, no):
        beg = (
            ...
            pd.Timedelta('1s'))
        if no == 0:
            return self.data[np.bitwise_and(self.data.index >= beg +
                                            pd.Timedelta('122 days 06:36:00').total_seconds(), self.data.index <= beg +
                                            pd.Timedelta('132 days 06:36:00').total_seconds())]
        elif no == 1:
            return self.data[np.bitwise_and(self.data.index >= beg +
                                            pd.Timedelta('617 days 06:36:00').total_seconds(), self.data.index <= beg +
                                            pd.Timedelta('627 days 06:36:00').total_seconds())]
        elif no == 2:
            return self.data[np.bitwise_and(self.data.index >= beg +
                                            pd.Timedelta('1340 days 06:36:00').total_seconds(), self.data.index <= beg +
                                            pd.Timedelta('1350 days 06:36:00').total_seconds())]
    ...
    def prepare(data, noise=False):
        ...
        if noise:
            ...
            data.Global_active_power = data.Global_active_power + \
                0.001 * rng.rand(N, 1)
...
class Traffic(object):
    ...
    def load_raw(file_objs):
        ...
        for split in ('train', 'test'):
            ...
            l = np.array(
                list(map(int, file_objs['PEMS_' + split + 'labels'].readline()[1:-2].split())))
    ...
    @staticmethod
    def diff(df):
        left = df[['DoW', 'Sec of Day']][np.mod(
            np.arange(df.index.size), 144) != 0]
        right = (df.values[1:, 2:] - df.values[:-1, 2:]
                 )[np.mod(np.arange(1, df.index.size), 144) != 0, :]
        return pd.DataFrame(
            np.hstack([left.values, right]), columns=df.columns)
    ...
    def plot(
        self, split="train", dow=[
            0, 1], sensor=3, bins=[
            144, 60], filename=None):
        import matplotlib.pyplot as plt
        import calendar

        xticks = np.arange(0, 144, 20)
        if isinstance(dow, int):
            values = [Traffic.stackup_smp(Traffic.equalize_smp_size(self.data[split]))[
                1][dow * 144:(dow + 1) * 144, sensor:(sensor + 1), :]]
            dow = [dow]
            if False:
                plt.hist2d(
                    np.repeat(
                    ...
                    out.flatten(),
                    bins=bins)
                plt.xlabel("time of day on %s" %
                           calendar.day_name[(dow + 6) % 7])
            ...
            values = [Traffic.stackup_smp(Traffic.equalize_smp_size(self.data[split]))[
                1][day * 144:(day + 1) * 144, sensor:(sensor + 1), :] for day in dow]
        if True:
            fig, ax = plt.subplots(
                1, len(values), figsize=(
                    6 * len(values), 3), sharey=True)
            if len(values) == 1:
                ax = [ax]
            for (i, out) in enumerate(values):
                ax[i].hist2d(np.repeat(np.arange(out.shape[0]), out.shape[2], 0), out.flatten(
                ), bins=bins, range=[[0, 143], [0, 0.25]])
                ax[i].set_xlabel("time of day on %s" %
                                 calendar.day_name[(dow[i] + 6) % 7])
                if i is 0:
...
class Mountain(object):
    def __init__(self, root='~/Documents/datasets/'):
        ...
        self.data = DataPrep.get_data(
            "https://github.com/ericlee0803/GP_Derivatives/raw/master/demos/Maps/MtSH.mat",
            None,
            Mountain.load_raw,
            pickle_name)
        self.measurement_interval_in_sec = 600
...
class Jura(object):
    ...
    def __init__(self, root='~/Documents/datasets/'):
        ...
        self.data = DataPrep.get_data(
            "https://drive.google.com/uc?export=download&id=189FhW0h8iz7BQSDyXH0QSL3cFFFyy9Ln",
            None,
            Jura.load_csv,
            pickle_name,
            "All_data.csv")
...
class ToyExample(object):
    def nonlinear_forward_mean(self, x):
        if len(x.shape) == 2:
            return np.stack(
                np.apply_along_axis(
                    self.nonlinear_forward_mean,
                    1,
                    x),
                axis=0).reshape(
                x.shape[0],
                x.shape[1] * 2)
        # return np.vstack((np.sin(x*4)+np.sin(x*3)*2, np.cos(x*2)+
        # np.cos(x*3)+3)).T
    ...
    def draw_conditioned_on(self, x, samps_per_input):
        ...
        return means + np.random.randn(means.size * samps_per_input).reshape(
            means.shape[0], means.shape[1], samps_per_input) * sds

    def cond_distr(self, inp):
        assert (len(inp.shape) == 1)
        return multivariate_normal(
            self.nonlinear_forward_mean(inp).flatten(), np.diag(
                self.nonlinear_forward_sd(inp).flatten()))
    ...
    def __init__(self, nsamps_x=500, nsamps_cond=20):
        ...
        self.y = self.y[:, :, None] + np.random.randn(self.y.size * nsamps_cond).reshape(
            self.y.shape[0], self.y.shape[1], -1) * self.nonlinear_forward_sd(self.x)[:, :, None]
