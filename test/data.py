    ...
    def get_data(url, md5, preparation_func, pickle_name, *contained_file_paths):
        ...
        if not exists(pickle_name):
            rval = preparation_func(
                DataPrep.download_check_unpack(url, md5, *contained_file_paths)
            )
...
class Power(object):
    def __init__(self, root="~/Documents/datasets/"):
        ...
        self.data = DataPrep.get_data(
            ...
            lambda file_objs: Power.prepare(
                Power.load_raw(file_objs["household_power_consumption.txt"])
            ),
            pickle_name,
    ...
    def get_ihgp_window(self, no):
        beg = (
            pd.to_datetime("2006-12-16 17:24:00") - pd.Timestamp("1970-01-01")
        ) // pd.Timedelta("1s")
        if no == 0:
            return self.data[
                np.bitwise_and(
                    self.data.index
                    >= beg + pd.Timedelta("122 days 06:36:00").total_seconds(),
                    self.data.index
                    <= beg + pd.Timedelta("132 days 06:36:00").total_seconds(),
                )
            ]
        elif no == 1:
            return self.data[
                np.bitwise_and(
                    self.data.index
                    >= beg + pd.Timedelta("617 days 06:36:00").total_seconds(),
                    self.data.index
                    <= beg + pd.Timedelta("627 days 06:36:00").total_seconds(),
                )
            ]
        elif no == 2:
            return self.data[
                np.bitwise_and(
                    self.data.index
                    >= beg + pd.Timedelta("1340 days 06:36:00").total_seconds(),
                    self.data.index
                    <= beg + pd.Timedelta("1350 days 06:36:00").total_seconds(),
                )
            ]
...
class Traffic(object):
        ...
        for split in ("train", "test"):
            ...
            l = np.array(
                list(
                    map(
                        int,
                        file_objs["PEMS_" + split + "labels"].readline()[1:-2].split(),
                    )
                )
            )
            ...
            d = np.array(
                [
                    [list(map(float, r.split())) for r in l[1:-2].split(";")]
    ...
    def plot(self, split="train", dow=[0, 1], sensor=3, bins=[144, 60], filename=None):
        ...
        if isinstance(dow, int):
            values = [
                Traffic.stackup_smp(Traffic.equalize_smp_size(self.data[split]))[1][
                    dow * 144 : (dow + 1) * 144, sensor : (sensor + 1), :
                ]
            ]
            dow = [dow]
            if False:
                plt.hist2d(
                    np.repeat(np.arange(out.shape[0]), out.shape[2], 0),
                    out.flatten(),
                    bins=bins,
                )
            ...
            values = [
                Traffic.stackup_smp(Traffic.equalize_smp_size(self.data[split]))[1][
                    day * 144 : (day + 1) * 144, sensor : (sensor + 1), :
                ]
                for day in dow
            ]
        if True:
            fig, ax = plt.subplots(
                1, len(values), figsize=(6 * len(values), 3), sharey=True
            )
            if len(values) == 1:
                ax = [ax]
            for (i, out) in enumerate(values):
                ...
                ax[i].set_xlabel(
                    "time of day on %s" % calendar.day_name[(dow[i] + 6) % 7]
                )
                if i is 0:
...
class Mountain(object):
    ...
    def load_raw(file_objs):
        ...
        return {
            "y": mt["mth_points"].T[0].reshape(ny, nx),
...
class ToyExample(object):
    def nonlinear_forward_mean(self, x):
        if len(x.shape) == 2:
            return np.stack(
                np.apply_along_axis(self.nonlinear_forward_mean, 1, x), axis=0
    ...
    def nonlinear_forward_sd(self, x):
        if len(x.shape) == 2:
            return np.stack(
                np.apply_along_axis(self.nonlinear_forward_sd, 1, x), axis=0
            ).reshape(x.shape[0], x.shape[1] * 2)
    ...
    def draw_conditioned_on(self, x, samps_per_input):
        ...
        return (
            means
            + np.random.randn(means.size * samps_per_input).reshape(
                means.shape[0], means.shape[1], samps_per_input
            )
            * sds
        )
    ...
    def cond_distr(self, inp):
        ...
        return multivariate_normal(
            self.nonlinear_forward_mean(inp).flatten(),
            np.diag(self.nonlinear_forward_sd(inp).flatten()),
        )
    ...
    def __init__(self, nsamps_x=500, nsamps_cond=20):
        ...
        self.y = (
            self.y[:, :, None]
            + np.random.randn(self.y.size * nsamps_cond).reshape(
                self.y.shape[0], self.y.shape[1], -1
            )
            * self.nonlinear_forward_sd(self.x)[:, :, None]
        )
