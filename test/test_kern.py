...
def test_rbf():
    ...
    tests = []
    for ls in np.linspace(0.9, 2.0, 5):
        gauss_ref = norm(loc=0.0, scale=ls)
        lapl_ref = laplace(loc=0.0, scale=ls)
        ...
        for shape in np.linspace(0.9, 2.0, 5):
            gg_ref = gennorm(beta=shape, scale=ls)
            tests.append(
                (
                    f"GG__sh_{shape}__sc_{ls}",
...
def test_SplitDimsKernel():
    (intervals, kernels) = (
        [0, 2, 5],
        [GenGaussKernel.make_gauss(0.1), GenGaussKernel.make_gauss(1)],
    )
