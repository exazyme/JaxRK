...
def location_mixture_logpdf(
    samps, locations, location_weights, distr_at_origin, contr_var=False, variant=1
):
    diff = samps - locations[:, np.newaxis, :]
    lpdfs = distr_at_origin.logpdf(
        diff.reshape([np.prod(diff.shape[:2]), diff.shape[-1]])
...
class categorical(object):
    def __init__(self, p):
        ...
        self.cum_lp = np.array(
            [logsumexp(self.lp[:i]) for i in range(1, len(self.lp) + 1)]
        )
    ...
    def rvs(self, size=1, indic=False):
        assert size >= 0
        return np.array([self.ppf(uniform.rvs(), indic=indic) for _ in range(size)])
...
class mixt(object):
    ...
    def logpdf(self, x):
        comp_logpdf = np.array(
            [
                self.dist_cat.logpdf(i) + self.comp_dist[i].logpdf(x)
    ...
    def rvs(self, num_samples=1):
        rval = np.array(
            [self.comp_dist[i].rvs(1) for i in self.dist_cat.rvs(num_samples)]
        )
