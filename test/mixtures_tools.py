...
def location_mixture_logpdf(
    ...
    lpdfs = distr_at_origin.logpdf(diff.reshape(
        [np.prod(diff.shape[:2]), diff.shape[-1]])).reshape(diff.shape[:2])
    logprop_weights = log(
        location_weights /
        location_weights.sum())[
        :,
        np.newaxis]
...
class categorical(object):
    def __init__(self, p):
        ...
        self.cum_lp = np.array([logsumexp(self.lp[:i])
                               for i in range(1, len(self.lp) + 1)])
...
class mixt(object):
    ...
    def logpdf(self, x):
        comp_logpdf = np.array([self.dist_cat.logpdf(
            i) + self.comp_dist[i].logpdf(x) for i in range(len(self.comp_dist))])
    ...
    def rvs(self, num_samples=1):
        rval = np.array([self.comp_dist[i].rvs(1)
                        for i in self.dist_cat.rvs(num_samples)])
