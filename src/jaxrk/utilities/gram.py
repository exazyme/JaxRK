...
def rkhs_gram_cdist(
    G_ab: np.array, G_a: np.array = None, G_b: np.array = None, power: float = 2.0
):
    ...
    if G_a is None or G_b is None:
        ...
        ), "Either none or both of G_repr, G_orig should be None"
...
def rkhs_gram_cdist_ignore_const(G_ab: np.array, G_b: np.array, power: float = 2.0):
...
def rkhs_gram_cdist_unchecked(
    G_ab: np.array, G_a: np.array, G_b: np.array, power: float = 2.0
):
